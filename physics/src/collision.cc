...
static std::optional<Contact> CircleCircleContact(core::Index<Collider> a,
  ...
  auto& cA = get_collider_at(a);
  auto& cB = get_collider_at(b);
  ...
  core::Vec2F AB = (get_collider_at(contact.collider_idxB).body().position + get_collider_at(contact.collider_idxB).offset)
                 - (get_collider_at(contact.collider_idxA).body().position + get_collider_at(contact.collider_idxA).offset);
...
static std::optional<Contact> CircleAABBContact(core::Index<Collider> circIdx,
                                                ...
                                                bool circleIsA) {
  auto& cC = get_collider_at(circIdx);
  auto& cR = get_collider_at(aabbIdx);
...
std::optional<Contact> GenerateContact(core::Index<Collider> idxA,
  ...
  auto& cA = get_collider_at(idxA);
  auto& cB = get_collider_at(idxB);
...
void ResolveCollision(Contact& contact, float restitution) {
  auto& colA = get_collider_at(contact.collider_idxA);
  auto& colB = get_collider_at(contact.collider_idxB);
  Body& A = colA.body();
  Body& B = colB.body();
