...
TEST(Vec2, Add)
{
  ...
  core::Vec2I vec4(6, 3);
  const core::Vec2I vec5 = vec4 += vec2 * 2;
  EXPECT_EQ(vec3.x, 10);
  EXPECT_EQ(vec3.y, 8);
  EXPECT_EQ(vec4.x, 14);
  EXPECT_EQ(vec4.y, 13);
...
TEST(Vec2, Muli)
{
  ...
  core::Vec2I vec4(2, 6);
  core::Vec2I vec5 = vec4 *= 2;
  EXPECT_EQ(vec4.x, 2*2);
  EXPECT_EQ(vec4.y, 6*2);
  EXPECT_EQ(vec5.x, 2*2);
  EXPECT_EQ(vec5.y, 6*2);
