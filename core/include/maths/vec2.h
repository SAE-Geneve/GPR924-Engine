...
//Concept is a named set of requirements that a type must satisfy to be used in a given context
//This concept checks if the type is a vector 2 and if both points of the vectors match to the type T
...
template <typename Vector2T, typename T>
//This concept checks if both points of the vector 2 can be converted to the type T
...
struct Vec2 {
  ...
  //nodiscard makes it so the result of the function is not ignored (Safety purposes)
  //constexpr makes it so that the result can be determined during compile time
  //operator overloads the normal use of the operator in the case it is used for a Vec2 struct
  ...
  [[nodiscard]] Vec2 RotateX(const Radian angle) const requires std::floating_point<T>
  {
  ...
  [[nodiscard]] Vec2 RotateY(const Radian angle) const requires std::floating_point<T>
  {
    const T c = std::cos(static_cast<float>(angle));
    const T new_x = x * c;
    return Vec2(new_x, y);
  ...
  [[nodiscard]] static constexpr Vec2 Lerp(const Vec2 v0, const Vec2 v1, const float t) requires std::is_floating_point_v<T>
  {
    return Vec2{
        v0.x * (1 - t) + v1.x * t,
        v0.y * (1 - t) + v1.y * t
  ...
  [[nodiscard]] static Vec2 Slerp(const Vec2 v0, const Vec2 v1, const float t) requires std::is_floating_point_v<T>
  {
    T dot = v0.Dot(v1);
  ...
  [[nodiscard]] static constexpr Vec2 Projection(const Vec2 v, const Vec2 onto)
  {
  ...
  [[nodiscard]] static constexpr Vec2 Reflection(const Vec2 v, const Vec2 normal)
  {
