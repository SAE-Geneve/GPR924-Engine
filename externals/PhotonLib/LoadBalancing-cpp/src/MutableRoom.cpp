...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		MutableRoom::MutableRoom(const JString& name, const Hashtable& properties, Client* pClient, const JVector<JString>& propsListedInLobby, int playerTtl, int emptyRoomTtl, bool suppressRoomEvents, const JVector<JString>* pPlugins, bool publishUserID, const JString& lobbyName, nByte lobbyType, const JVector<JString>& expectedUsers)
		...
		const JString& MutableRoom::getLobbyName(void) const
		{
			return mLobbyName;
		}

		const nByte MutableRoom::getLobbyType(void) const
		{
			return mLobbyType;
		}
