	...
	namespace LoadBalancing
	{
		...
		MutableRoom& Client::getCurrentlyJoinedRoom(void)
		{
			if(!mpCurrentlyJoinedRoom)
				mpCurrentlyJoinedRoom = createMutableRoom(L"", Hashtable(), JVector<JString>(), 0, 0, false, NULL, false, JString(), LobbyType::DEFAULT, JVector<JString>());
...
#	endif
		...
		bool Client::opCreateRoom(const JString& gameID, const RoomOptions& options, const JVector<JString>& expectedUsers)
		{
			...
			mpCurrentlyJoinedRoom = createMutableRoom(gameID, roomProps, options.getPropsListedInLobby(), options.getPlayerTtl(), options.getEmptyRoomTtl(), options.getSuppressRoomEvents(), options.getPlugins(), options.getPublishUserID(), options.getLobbyName(), options.getLobbyType(), expectedUsers);
			destroyMutableRoom(oldRoom);
			mRoomOptions = options;
		...
		/**
		   ...
		   In the case that a new room gets created, that room will be attached to the lobby that you have specified in the parameters 'lobbyName' and 'lobbyType'. Leave the lobby name empty to attach the room to the
		   lobby in which you reside at the time of room creation. If you are in no lobby, then the default lobby is used.
		   @note
		   The values of RoomOptions::getLobbyName() and RoomOptions::getLobbyType() are ignored and the values of the separate parameters 'lobbyName' and 'lobbyType' are used for both, looking for an existing room and creation of a new room. 
		   @remarks
		   A Client instance can only be inside one room at a time. Therefor this operation will fail and return false, if the client is already inside another game room. Any lobby the client
		   currently resides in will implicitly be left when entering a game room.
		   @remarks
		...
		bool Client::opJoinRandomOrCreateRoom(const JString& gameID, const RoomOptions& options, const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const JString& lobbyName, nByte lobbyType, const JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			...
			MutableRoom* oldRoom = mpCurrentlyJoinedRoom;
			mpCurrentlyJoinedRoom = createMutableRoom(gameID, roomProps, options.getPropsListedInLobby(), options.getPlayerTtl(), options.getEmptyRoomTtl(), options.getSuppressRoomEvents(), options.getPlugins(), options.getPublishUserID(), lobbyName, lobbyType, expectedUsers);
			destroyMutableRoom(oldRoom);
			mRoomOptions = options;
			if(mState != PeerStates::Joining)
		...
		bool Client::opJoinRoom(const JString& gameID, bool rejoin, int cacheSliceIndex, const JVector<JString>& expectedUsers)
		{
			...
			MutableRoom* oldRoom = mpCurrentlyJoinedRoom;
			mpCurrentlyJoinedRoom = createMutableRoom(gameID, Hashtable(), JVector<JString>(), 0, 0, false, NULL, false, JString(), LobbyType::DEFAULT, expectedUsers);
			destroyMutableRoom(oldRoom);
		...
		bool Client::opJoinRandomRoom(const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const JString& lobbyName, nByte lobbyType, const JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			...
			MutableRoom* oldRoom = mpCurrentlyJoinedRoom;
			mpCurrentlyJoinedRoom = createMutableRoom(L"", Utils::stripToCustomProperties(customRoomProperties), Common::JVector<Common::JString>(), 0, 0, false, NULL, false, lobbyName, lobbyType, expectedUsers);
			destroyMutableRoom(oldRoom);
			if(mState != PeerStates::Joining)
...
#	pragma GCC diagnostic pop
		...
		void Client::onEvent(const EventData& eventData)
		{
			...
			{
			case EventCode::ERROR_INFO:
				{
					EGLOG(DebugLevel::ERRORS, L"%ls", ValueObject<JString>(eventData.getParameterForCode(ParameterCode::INFO)).getDataCopy().cstr());
				}
				break;
		...
		void Client::onArrivalAndAuthentication(void)
		{
			...
			else if(mState == PeerStates::ConnectedToGameserver)
			{
				...
				if(mLastJoinType == JoinType::CREATE_ROOM)
					opCreateRoom(mRoomName, mRoomOptions, mpCurrentlyJoinedRoom->getExpectedUsers());
				else if(mLastJoinType == JoinType::JOIN_OR_CREATE_ROOM)
					opJoinOrCreateRoom(mRoomName, mRoomOptions, mLastCacheSliceIndex, mpCurrentlyJoinedRoom->getExpectedUsers());
				else if(mLastJoinType == JoinType::JOIN_RANDOM_OR_CREATE_ROOM)
					opJoinOrCreateRoom(mRoomName, mRoomOptions, 0, mpCurrentlyJoinedRoom->getExpectedUsers());
		...
		MutableRoom* Client::createMutableRoom(const JString& name, const Hashtable& properties, const JVector<JString>& propsListedInLobby, int playerTtl, int emptyRoomTtl, bool suppressRoomEvents, const JVector<JString>* pPlugins, bool publishUserID, const JString& lobbyName, nByte lobbyType, const JVector<JString>& expectedUsers)
		{
			mpMutableRoomFactory = getMutableRoomFactory();
			return mpMutableRoomFactory->create(name, properties, this, propsListedInLobby, playerTtl, emptyRoomTtl, suppressRoomEvents, pPlugins, publishUserID, lobbyName, lobbyType, expectedUsers);
		}
