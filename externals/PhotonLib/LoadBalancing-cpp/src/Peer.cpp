...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		{
			OperationRequestParameters op = enterRoomImplementation(createIfNotExists?&RoomOptions(options).setLobbyName(lobbyName).setLobbyType(lobbyType):NULL, Hashtable(), expectedUsers, true);
			if(gameID.length())
				op.put(ParameterCode::ROOM_NAME, ValueObject<JString>(gameID));
			if(createIfNotExists)
				op.put(ParameterCode::JOIN_MODE, ValueObject<nByte>(JoinMode::CREATE_IF_NOT_EXISTS));
